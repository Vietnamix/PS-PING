...
function Open-Browser {
    ...
    if ($found -and ($found -like "*msedge.exe" -or $found -like "*chrome.exe")) {
        ...
        try {
            $proc = Start-Process -FilePath $found -PassThru -ArgumentList @(
                "--app=$fileUrl",
...
<title>NN Ping Monitor v4.75</title>
$chartJsTag
<style>
:root{
...
var curSession=0;
var MAXKEEP=3000;   // doit correspondre a $MaxKeep cote PowerShell
...
Write-Host "   PS-PING - Ping Monitor v4.75"            -ForegroundColor White
...
# Horloges de cadence : ping, ecriture, et surveillance de la fenetre
$lastWrite = Get-Date
$lastPing  = Get-Date
$lastAlive = Get-Date
$AliveEvery = 5   # secondes entre deux verifications de la fenetre
...
try {
    ...
    while ($true) {
        ...
        # --- Surveillance de la fenetre (auto-arret) ---
        # Si le navigateur a ete lance en mode app (processus surveillable)
        # et que sa fenetre a ete fermee, on quitte proprement la boucle.
        if ($script:browserProc) {
            $sinceAlive = ((Get-Date) - $lastAlive).TotalSeconds
            if ($sinceAlive -ge $AliveEvery) {
                $lastAlive = Get-Date
                $closed = $false
                try { if ($script:browserProc.HasExited) { $closed = $true } } catch { $closed = $true }
                if ($closed) {
