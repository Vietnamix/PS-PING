...
# ------------------------------------------------------------
# Open-Browser : ouvre le rapport HTML. Cherche d'abord un
#   navigateur moderne connu (Edge/Chrome/Firefox) pour eviter
#   qu'Internet Explorer ne s'ouvre par defaut (IE bloque le JS
#   local). A defaut, ouvre avec le navigateur systeme par defaut.
# ------------------------------------------------------------
...
function Write-HtmlOnce {
    param([string]$target)
    $dataFileJs = $dataFile -replace '\\','\\\\'   # échappe \ pour le JS
...
<title>NN Ping Monitor v4.7</title>
$chartJsTag
<style>
:root{
...
//   <script> (avec ?_=timestamp pour casser le cache navigateur).
//   Quand il s'execute, il (re)definit window.PD ; on synchronise
//   alors le cache local aD :
//     - si plus d'items qu'avant  -> ajoute seulement le DELTA
//       (evite de tout re-parser a chaque cycle) ;
//     - si moins (script relance)  -> recharge tout ;
//   puis met a jour le bandeau (uu), la scrollbar (usb) et, en
//   live uniquement, le graphe (uc). Ne fait rien si l'utilisateur
//   est en train de taper (pr=true).
// ============================================================
function loadData(){
  ...
  s.id='ds';s.src=DJP+'?_='+Date.now();          // anti-cache
  s.onload=function(){
    if(window.PD){
      var d=window.PD;
      if(d.items.length>ldc){                    // nouveaux points -> delta
        d.items.slice(ldc).forEach(function(p){
          aD.push({t:parseInt(p.t,10),v:parseInt(p.v,10)});
        });
        ldc=d.items.length;
        if(aD.length>3000)aD=aD.slice(aD.length-3000);  // plafond cote client
      } else if(d.items.length<ldc){             // script relance -> recharge tout
...
# --- MAIN ---
# ------------------------------------------------------------
# Sequence : (1) horodatage de depart, (2) banniere console,
# (3) ping de test, (4) generation du HTML + premieres donnees,
# (5) ouverture navigateur, (6) BOUCLE infinie de monitoring.
# Le bloc finally garantit une derniere ecriture propre a l'arret.
# ============================================================
$script:startMs = Get-NowMs   # reference de depart (epoch ms)

Write-Host ""
Write-Host "  Mode : $($ExecutionContext.SessionState.LanguageMode)" -ForegroundColor Cyan
Write-Host "  ============================================" -ForegroundColor Cyan
Write-Host "   PS-PING - Ping Monitor v4.7"             -ForegroundColor White
...
# Genere la page UNE fois, ecrit un premier jeu de donnees, ouvre le navigateur
...
# Horloges de cadence : un timer pour les pings, un pour l'ecriture
$lastWrite = Get-Date
$lastPing  = Get-Date

try {
    # ----- BOUCLE PRINCIPALE -----
    # Tourne toutes les 100 ms. Deux cadences independantes :
    ...
    while ($true) {
        ...
        if ($sinceLW -ge $WriteEvery) {
            ...
            # Ligne de log console synthetique
            $st      = Calc-Stats
            $elapsed = Get-ElapsedSec
            $hh      = [int]($elapsed / 3600)
